...
class runtime(gen_basic):
  ...
  def return_config_packet_list_local(self, page_num_dict, connection_list, operators):
    ...
    self.shell.replace_lines(self.bit_dir+'/host.cpp', {'configg.h': include_str}) 
  ...
  def run(self, operators):
    ...
    self.shell.cp_file('common/script_src/gen_runtime_'+self.prflow_params['board']+'.sh', self.bit_dir+'/gen_runtime.sh')
    tmp_dict = {'Xilinx_dir'          : 'source '+self.prflow_params['Xilinx_dir'],
    ...
    tmp_list = ['#!/bin/bash -e', './load.exe dynamic_region.xclbin']
